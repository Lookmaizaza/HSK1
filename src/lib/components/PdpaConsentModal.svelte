...
<script lang="ts">
	...
	async function handleAccept(requestMic = true) {
		...
		const userId = user?.id ? String(user.id) : 'usr_uuid_local';
		const userStorageKey = `yupakjeen_pdpa_consent_user_${userId}`;

		// Save consent record locally per user
		const consentData = {
			userId,
			termsAccepted,
			audioConsent,
		...
		>
			...
			<div class="space-y-2">
				...
				<h2 id="pdpa-modal-title" class="text-xl sm:text-2xl font-black tracking-tight text-foreground">
					ยินดีต้อนรับ {user?.username ? `คุณ ${user.username}` : ''} สู่ระบบ "语 ปากจีน"
				</h2>
